...
fn read_data_transfer(
    ...
    let okr = okr_result.map_err(|_| ScpiErrors::BulkInTransferError)?;
...
fn read_data(
    ...
    let buffer_size = if req_buffer_size > max_transfer_size || req_buffer_size == 0 {
        return Err(ScpiErrors::BadBufferSize);
    } else {
        req_buffer_size
    };
    ...
    let ok2 = ok2_results.map_err(|_| ScpiErrors::BulkOutTransferError)?;
...
pub fn send_command_raw(usbtmc: &mut Usbtmc, command: &str) -> Result<Vec<u8>, ScpiErrors> {
    ...
    let ok = ok_results.map_err(|_| ScpiErrors::BulkOutTransferError)?;
...
pub fn send_command(usbtmc: &mut Usbtmc, command: &str) -> Result<String, ScpiErrors> {
